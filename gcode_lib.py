...
# VTK is optional — rendering functions gracefully degrade when absent.
try:
...
def _needs_subprocess_render() -> bool:
    ...
    thread.  Always render in a subprocess on Darwin to isolate potential
    native crashes.
    """
...
#: ``PRINTER_PRESETS`` (after alias resolution).
...
def resolve_printer(name: str) -> str:
    ...
    Returns the canonical (upper-case) template key.
...
def derive_slicer_dimensions(
    ...
    Uses PrusaSlicer's auto-width formulas:

    - ``layer_height = nozzle_size × 0.5``
    - ``extrusion_width = nozzle_size × 1.125``

    Returns ``(layer_height, extrusion_width)`` rounded to 2 decimal places.
    """
...
def resolve_filament_preset(
    ...
    *,
    ...
    Explicit overrides (*nozzle_temp*, *bed_temp*, *fan_speed*) take
    precedence over preset defaults.  Unknown filament types fall back
    to safe defaults (210 °C / 60 °C / 100% fan).

    Returns ``{"nozzle_temp": int, "bed_temp": int, "fan_speed": int}``.
    """
...
def gcode_ext(binary: bool = True) -> str:
    ...
    Returns ``".bgcode"`` when *binary* is ``True`` (default),
    ``".gcode"`` when *binary* is ``False``.
    """
...
def safe_filename_part(value: str) -> str:
    ...
    Strips path separators (``/``, ``\\``), parent-directory sequences
    (``..``), and null bytes so that the result cannot escape the intended
    output directory when used as part of a filename.
    """
